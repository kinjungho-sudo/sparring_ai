...
export async function POST(req: NextRequest) {
  ...
  const { debate_id, topic, currentRound, totalRounds, language, history, blue_config, model = 'claude-haiku-4-5-20251001' } = body
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        controller.close()
        logApiCall({ debate_id, endpoint: '/api/debate/blue', duration_ms: Date.now() - startTime, status: streamError ? 'error' : 'success', error_message: streamError })
      }
    },
  })
